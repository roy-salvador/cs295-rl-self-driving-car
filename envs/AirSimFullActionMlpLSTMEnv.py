'''
    AirSim Environment with Full Action Controls [Steering, Throttle, Brake, Gear]
'''
...
POSSIBLE_WAY_POINTS = [(airsim.Vector3r(30 , 0, -1), airsim.utils.to_quaternion(0,0,0))] 
...
class AirSimEnv(Env) :
    ...
    def __init__(self, 
        ...
        self.action_space = spaces.Box(low=np.array([-1,0,0,-1]), high=np.array([1,1,1,1]), dtype=np.float32)
    ...
    def update_current_state(self) :
        '''
        Observation States
            self.car_state - linear and anguar velocity, rpm speed
    ...
    def get_reward_status(self) :
        ...
        reward += ((self.max_goal_distance - dist)*20 / (self.max_goal_distance - self.goal_threshold))
    ...
    # An action is given by a vector [Steering, Throttle, Brake, Gear]
