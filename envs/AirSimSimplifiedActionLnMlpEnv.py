...
MAX_GOAL_DISTANCE = 100 # Farthest distance in meters from the goal which can be rewarded
...
class  AirSimEnv(Env) :
    ...
    def __init__(self, 
        ...
        # Value Range for the orientation (w,x,y,z) and speed, distance and angle from current waypoint 
        low = [-1, -1, -1, -1, 0, 0, 0]
        high = [1, 1, 1, 1, 1, 1000, 2*math.pi]
    ...
    def update_current_state(self) :
        ...
        Observation States
            ...
            euclidean distance and angle of the waypoint wrt the agent car
        ...
        self.tcar_car_pos = [tcar_state.position.x_val, tcar_state.position.y_val, tcar_state.position.z_val]
        dist = self._euclidean_distance(self.car_pos, self.tcar_car_pos)
        obs.append(dist)        
        
        # orientation of the way point wrt to the car
        self.tcar_yaw = self._get_yaw_angle_rad(self.car_pos, self.tcar_car_pos)
        #print('HERE ' + str(self.tcar_yaw))
        obs.append(self.tcar_yaw)
    ...
    def get_reward_status(self) :
        ...
        reward += -abs(self.car_controls.steering) 
            
        # Intermediate penalty to encourage to reach goal in smaller amount of time
        reward += - 1
            
        # Intermediate reward proportional to euclidean distance from way point. 
        dist = self.current_state[5] 
        ...
        reward += math.cos( self.tcar_yaw -  airsim.utils.to_eularian_angles(self.client.getCarState().kinematics_estimated.orientation)[2] )
        ...
        if self.time_steps_left == 0 :
            ...
            if dist <= self.goal_threshold :
                ...
                if self.current_goal >= len(self.goals) :
                    ...
                    # Reward for finishing faster
                    reward += (10 * self.current_goal * self.time_steps_left)
        ...
        # Continuous reward for getting previous waypoints
        reward += (10 * self.current_goal)
